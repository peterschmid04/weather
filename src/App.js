...
export default function WeatherApp() {
  ...
  const getNextSevenDays = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push(date.toLocaleDateString("en-EN", options));
    }
    return days;
  };
  ...
  const [forecastDays] = useState(getNextSevenDays());
        ...
        setHighlights([
          {
            ...
            value: uvData.value,
            unit: "",
            status: getStatusUV(uvData.value),
          },
          {
            title: "Wind Status",
            value: data.wind.speed,
            unit: "km/h",
            status: getStatusWind(data.wind.speed),
          },
          { title: "Sunrise & Sunset", up: `${sunrise}`, down: `${sunset}` },
          {
            ...
            unit: "%",
            status: getStatusHumidity(data.main.humidity),
          },
          {
            title: "Visibility",
            value: (data.visibility / 1000).toFixed(1),
            unit: "",
            status: getStatusVisibility((data.visibility / 1000).toFixed(1)),
          },
          {
            title: "Air Quality",
            value: airQualityData.list[0].main.aqi,
            unit: "",
            status: getStatusAirquality(airQualityData.list[0].main.aqi),
          }, // Air quality index
        ]);
        setError("");
      } catch (error) {
